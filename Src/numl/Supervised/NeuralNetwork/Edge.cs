...
namespace numl.Supervised.NeuralNetwork
{
    ...
    public class Edge : Data.IEdge
    {
        ...
        /// <param name="epsilon">Seed value to use when randomly selecting a weight (ignored when <paramref name="weight"/> is supplied).
        ...
        public static Edge Create(Neuron source, Neuron target, double weight = double.NaN, double epsilon = double.NaN)
        {
            ...
            if (!double.IsNaN(weight))
                ...
                e.Weight = Edge.GetWeight(epsilon);
        ...
        /// <returns></returns>
        public static double GetWeight(double epsilon)
        {
            return Sampling.GetUniform(-epsilon, epsilon);
        }
